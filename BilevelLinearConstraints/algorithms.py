...
from stochastic_bilevel_comparison import gen_stochastic_problem
...
class DsBlo(Algorithm):
    ...
    def run(self, x, y):
        ...
        # Check if Aact or Bact is None
        if Aact is None or Bact is None:
            print("No active constraints found. Skipping this iteration.")
            return  # Exit the run method or handle it as needed
...
def test_ds_blo_algorithm():
    """Test the DsBlo algorithm with a stochastic bilevel problem."""
    # Generate a stochastic bilevel problem
    problem = gen_stochastic_problem(xd=5, yd=20, m=10, seed=42)

    # Initialize the DsBlo algorithm
    ds_blo = DsBlo(problem, out_iter=50000, gamma1=0.1, gamma2=1.0, beta=0.9)

    # Initial values for x and y
    x_init = np.random.rand(problem.x_dim)
    y_init = np.random.rand(problem.y_dim)

    # Run the algorithm
    ds_blo.run(x_init, y_init)

    # Print results
    print("Algorithm run completed.")
    print(f"Final loss: {ds_blo.loss[-1] if ds_blo.loss else 'N/A'}")
    print(f"Final gradient norm: {ds_blo.gradF[-1] if ds_blo.gradF else 'N/A'}")

# Call the test function if this script is run directly
if __name__ == "__main__":
    test_ds_blo_algorithm()