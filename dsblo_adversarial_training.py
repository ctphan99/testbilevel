...
class DSBLOAdversarialTraining:
    ...
    def compute_implicit_gradient(self, inputs, targets, y_star, q):
        ...
        For DS-BLO, we use the stochastic gradient approximation with GPU optimizations
        """
        # Enable gradients for model parameters
        for param in self.model.parameters():
            param.requires_grad = True
        
        # Compute upper-level loss with mixed precision if available
        if self.scaler is not None:
            with torch.cuda.amp.autocast():
        ...
        noise = torch.randn_like(grad_x_flat, device=self.device) * self.perturbation_std
