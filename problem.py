...
class StronglyConvexBilevelProblem:
    ...
    def __init__(self, dim: int = 100, num_constraints: int = 3, noise_std: float = 0.01, device: str = 'cpu', seed: int = 42, strong_convex: bool = False):
        self.dim = dim
        ...
        if strong_convex:
            ...
            # Ensure positive semidefinite without strong convexity
            eigenvals, eigenvecs = torch.linalg.eigh(self.Q_upper)
            eigenvals = torch.clamp(eigenvals, min=0.1)  # Ensure reasonable positive eigenvalues
            self.Q_upper = eigenvecs @ torch.diag(eigenvals) @ eigenvecs.T
        ...
        if strong_convex:
            ...
            # Ensure positive semidefinite without strong convexity
            eigenvals, eigenvecs = torch.linalg.eigh(self.Q_lower)
            eigenvals = torch.clamp(eigenvals, min=0.1)  # Ensure reasonable positive eigenvalues
            self.Q_lower = eigenvecs @ torch.diag(eigenvals) @ eigenvecs.T
        ...
        # Tighter constraints to ensure they are active at the optimum
        self.b = torch.randn(num_constraints, device=device, dtype=self.dtype) * 0.05
        ...
        print(f"Upper level strong convexity: lam_min={upper_eigenvals.min():.3f}, lam_max={upper_eigenvals.max():.3f}")
        print(f"Lower level strong convexity: lam_min={lower_eigenvals.min():.3f}, lam_max={lower_eigenvals.max():.3f}")
    ...
    def _sample_instance_noise(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Sample independent noise for Q matrices
        Q_upper_noise = torch.randn(self.dim, self.dim, device=self.device, dtype=self.dtype) * self.noise_std
        Q_lower_noise = torch.randn(self.dim, self.dim, device=self.device, dtype=self.dtype) * self.noise_std
        
        # Make symmetric and add to base matrices
        Q_upper = self.Q_upper + (Q_upper_noise + Q_upper_noise.T) / 2.0
        Q_lower = self.Q_lower + (Q_lower_noise + Q_lower_noise.T) / 2.0
        
        return Q_upper, Q_lower
    def upper_objective(self, x: torch.Tensor, y: torch.Tensor, add_noise: bool = True) -> torch.Tensor:
        """Upper level objective with instance noise in Q matrix"""
        if add_noise:
            Q_upper, _ = self._sample_instance_noise()
        else:
            Q_upper = self.Q_upper
            
        term1 = 0.5 * (x - self.x_target) @ Q_upper @ (x - self.x_target)
        term2 = self.c_upper @ y
        return term1 + term2

    def lower_objective(self, x: torch.Tensor, y: torch.Tensor, add_noise: bool = True) -> torch.Tensor:
        """Lower level objective with instance noise in Q matrix - NO ZETA NOISE"""
        if add_noise:
            _, Q_lower = self._sample_instance_noise()
        else:
            Q_lower = self.Q_lower
            
        term1 = 0.5 * y @ Q_lower @ y
        linear_term = (self.c_lower + self.P.T @ x)
        return term1 + linear_term.sum()
    ...
    def solve_lower_level(self, x: torch.Tensor, max_iter: int = 200, tol: float = 1e-8,
                           y_linear_offset: Optional[torch.Tensor] = None,
                           allow_grad: bool = False, solver: str = 'pgd',
                           saa_batch: int = 0, saa_noise_std: Optional[float] = None) -> Tuple[torch.Tensor, Dict]:
        """
        Solve constrained lower level QP.

        Problem: min_y 0.5 * y^T Q_lower y + (c_lower + P^T x + (y_linear_offset) + \\bar{\\xi})^T y
                 s.t.  By ≥ Ax - b

        SAA (Sample Average Approximation): if saa_batch > 0, we draw iid noise vectors
        {\\xi_b}_b=1..B with \\xi_b ~ N(0, sigma^2 I), compute \\bar{\\xi} = (1/B) * sum_b \\xi_b,
        and add it to the linear term so the noise influences argmin_y.

        If y_linear_offset is provided (e.g., perturbation q), it is added to the linear term as well.
        """
        # Keep gradient path if allow_grad=True (for torch-based solvers), otherwise detach
        x_det = x if allow_grad else x.detach()

        solver_lower = (solver or 'pgd').lower()

        # Build torch tensors for PGD branch and diagnostics
        Q_t = (self.Q_lower + self.Q_lower.T) / 2.0

        # SAA noise mean for linear-in-y model
        xi_mean = torch.zeros(self.dim, device=self.device, dtype=self.dtype)
        if isinstance(saa_batch, int) and saa_batch > 0:
            sigma = float(saa_noise_std) if (saa_noise_std is not None) else float(self.noise_std)
            # Average B Gaussian samples
            xi_samples = torch.randn(saa_batch, self.dim, device=self.device, dtype=self.dtype) * sigma
            xi_mean = xi_samples.mean(dim=0)

        d_t = (self.c_lower + self.P.T @ x_det
               + (y_linear_offset if y_linear_offset is not None else 0.0)
               + xi_mean)
        B_t = self.B
        c_t = (self.A @ x_det - self.b)

        if solver_lower in ('pgd', 'projected_gradient', 'pgd_torch'):
            # Use projected gradient + KKT refinement (first-order anchor)
            y_kkt, info = self._solve_projected_gradient(Q_t, d_t, B_t, c_t, max_iter=max_iter, tol=tol)
            # Attach SAA diagnostics
            info = dict(info)
            info.update({'saa_batch': int(saa_batch), 'saa_noise_std': float(saa_noise_std) if saa_noise_std is not None else float(self.noise_std) if saa_batch > 0 else 0.0})
            return y_kkt, info

        # Otherwise, solve with CVXPy (default)
        Q = (Q_t.detach().cpu().numpy() + Q_t.detach().cpu().numpy().T) / 2.0
        d_vec = d_t.detach().cpu().numpy()
        B = B_t.detach().cpu().numpy()
        c_vec = c_t.detach().cpu().numpy()

        # Define and solve QP via CVXPy
        y_var = cp.Variable(self.dim)
        objective = cp.Minimize(0.5 * cp.quad_form(y_var, Q) + d_vec @ y_var)
        constraints = [B @ y_var >= c_vec]
        problem = cp.Problem(objective, constraints)
        if solver_lower in ('osqp', 'cvxpy_osqp', 'osqp_cvxpy'):
        ...
        stationarity_residual = (self.Q_lower @ y_star + (self.c_lower + self.P.T @ x_det + xi_mean)) - self.B.T @ lambda_dual
        ...
        info = {
            ...
            'saa_batch': int(saa_batch),
            'saa_noise_std': float(saa_noise_std) if saa_noise_std is not None else float(self.noise_std) if saa_batch > 0 else 0.0
        }
        return y_star, info
    ...
    def compute_gap(self, x: torch.Tensor, saa_batch: int = 0, saa_noise_std: Optional[float] = None) -> float:
        """
        Compute bilevel optimality gap using LL solution with optional SAA at the LL anchor.
        Gap = ||∇_x f(x,y*(x)) + [∇y*(x)]^T ∇_y f(x,y*(x))||

        If saa_batch > 0, the LL anchor y*(x) and all perturbed solves use SAA on the
        linear term of the LL objective with Gaussian noise std = saa_noise_std (default self.noise_std).
        """
        # Solve lower level with optional SAA
        y_star, ll_info = self.solve_lower_level(x, saa_batch=saa_batch, saa_noise_std=saa_noise_std)
        ...
        if not ll_converged or constraint_violation > 1e-6:
            print(f"   WARNING  LL solution quality: conv={ll_converged}, viol={constraint_violation:.2e}")

        # Compute direct gradient ∇_x f(x,y*)
        x_copy = x.clone().requires_grad_(True)
        y_copy = y_star.clone().requires_grad_(True)

        f_val = self.upper_objective(x_copy, y_copy, add_noise=True)
        ...
        for i in range(x.shape[0]):
            ...
            x_pert = x.clone()
            x_pert[i] += eps
            y_pert, _ = self.solve_lower_level(x_pert, saa_batch=saa_batch, saa_noise_std=saa_noise_std)
    ...
    def compute_stationarity_gap(self, x: torch.Tensor, y: torch.Tensor) -> float:
        """
        Compute stationarity gap: ||∇_x f(x,y) + ∇_y f(x,y)||
        This measures how close we are to a stationary point of the bilevel problem.
        """
        x_copy = x.clone().requires_grad_(True)
        y_copy = y.clone().requires_grad_(True)
        
        f_val = self.upper_objective(x_copy, y_copy, add_noise=True)
        grad_f_x = torch.autograd.grad(f_val, x_copy, create_graph=True, retain_graph=True)[0]
        grad_f_y = torch.autograd.grad(f_val, y_copy, create_graph=True, retain_graph=True)[0]
        
        stationarity_gap = float(torch.norm(grad_f_x + grad_f_y))
        return stationarity_gap

    def compute_optimality_gap(self, x_current: torch.Tensor, x_optimal: Optional[torch.Tensor] = None) -> float:
        """
        Compute optimality gap: f(x_current) - f(x_optimal)
        If x_optimal is not provided, we'll use a reference point or estimate.
        """
        if x_optimal is None:
            # For now, use a simple reference point (could be improved with better estimation)
            x_optimal = torch.zeros_like(x_current)
        
        f_current = self.true_bilevel_objective(x_current, add_noise=False)
        f_optimal = self.true_bilevel_objective(x_optimal, add_noise=False)
        
        optimality_gap = float(f_current - f_optimal)
    ...
    def compute_suboptimality_gap(self, x: torch.Tensor) -> float:
        """Compute suboptimality gap: F(x) - F(x*) where F(x) = f(x, y*(x))"""
        y_star_x, _ = self.solve_lower_level(x)
        F_x = self.upper_objective(x, y_star_x, add_noise=True)
        y_star_xstar, _ = self.solve_lower_level(self.x_target)
        F_xstar = self.upper_objective(self.x_target, y_star_xstar, add_noise=True)
        return float(F_x - F_xstar)

    def compute_suboptimality_gap_robust(self, x: torch.Tensor, num_samples: int = 10) -> float:
        """Compute robust suboptimality gap with multiple noise samples"""
        gaps = []
