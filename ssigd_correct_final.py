...
class CorrectSSIGD:
    ...
    def solve(self, T=1000, beta=0.01, x0=None, diminishing: bool = True, mu_F: float = None):
        ...
        for r in range(1, T + 1):  # 1-based iteration like DS-BLO
            ...
            if r <= 10 or r % 100 == 0:  # Log first 10 iterations and every 100th
                x_diff = torch.norm(x - x_new).item()
                print(f"  Iter {r:3d}: lr={lr_t:.8f}, x_new_norm={torch.norm(x_new).item():.6f}, x_final_norm={torch.norm(x).item():.6f}, diff={x_diff:.8f}")
