#!/usr/bin/env python3
"""
DS-BLO Fixed Implementation
Corrected version based on algorithms.py with proper parameter handling
"""
...
class DSBLOFixed:
    """
    DS-BLO Fixed Implementation with proper parameter handling
    """
    ...
    def __init__(self, problem: StronglyConvexBilevelProblem, device: str = 'cpu', dtype=torch.float64):
        ...
        print(f"DS-BLO Fixed Implementation")
    ...
    def optimize(self, x0: torch.Tensor, T: int, alpha: float) -> Dict:
        """
        Run DS-BLO optimization with fixed parameters
        """
        print("🚀 DS-BLO Fixed Algorithm")
        print("=" * 50)
        print(f"T = {T}, α = {alpha:.6f}")
        print()
        
        # Set parameters based on DS-BLO paper with better scaling
        delta_bar = alpha**3
        K = max(1, int(1.0 / delta_bar))
        gamma1 = K / delta_bar
        gamma2 = 4 * gamma1 * (1.0 + 2.0)
        beta = 0.9
        ...
        # Main optimization loop with better step size control
        for t in range(1, T + 1):
            # Update x_{t+1} = x_t - η_t * m_t with better step size
            grad_norm = torch.norm(m)
            eta = 1.0 / (gamma1 * grad_norm + gamma2)
            
            # Add step size clipping for stability
            ...
            g_norm = torch.norm(g)
            if g_norm > 1000:  # Clip large gradients
                g = g / g_norm * 1000
...
def main():
    """Test DS-BLO fixed implementation"""
    parser = argparse.ArgumentParser(description='DS-BLO Fixed Implementation')
    ...
    parser.add_argument('--plot-name', type=str, default='dsblo_fixed.png', help='Plot filename')
    
    args = parser.parse_args()
    
    print("=" * 80)
    print("DS-BLO FIXED IMPLEMENTATION TEST")
    print("=" * 80)
    ...
    ax1.set_title('DS-BLO Fixed Upper-level Loss')
    ...
    ax2.set_title('DS-BLO Fixed Hypergradient Norm')
