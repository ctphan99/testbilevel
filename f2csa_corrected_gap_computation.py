#!/usr/bin/env python3
"""
F2CSA CORRECTED GAP COMPUTATION - ACHIEVING SMOOTH AVERAGE GAP < 0.1
===================================================================

CRITICAL FIX: Use correct gap computation method in F2CSA algorithm
ROOT CAUSE: F2CSA was calling broken problem.compute_gap() method
SOLUTION: Implement proper gap computation directly in F2CSA algorithm

KEY INSIGHTS FROM DIAGNOSTIC:
- Lower-level solver IS responding to perturbations ✅
- Implicit gradient computation IS working ✅  
- Problem was in F2CSA algorithm calling wrong gap method ❌
- Need to use analytical Jacobian for inactive constraints ✅
"""
...
class F2CSACorrectedGapComputation:
    """
    F2CSA Algorithm with corrected gap computation
    
    Key fixes:
    - Use analytical Jacobian for inactive constraints
    - Implement proper implicit gradient computation
    - Fix gap computation method
    """
    ...
    def __init__(self, 
        ...
        if verbose:
            print(f"[CORRECTED GAP COMPUTATION] F2CSA Algorithm")
    ...
    def compute_gap_corrected(self, x: torch.Tensor) -> Tuple[float, float, float]:
        """
        Corrected gap computation using analytical Jacobian for inactive constraints
        """
        # Solve lower level
        ...
        # Check if constraints are inactive
        h_val = self.problem.A @ x - self.problem.B @ y_star - self.problem.b
        slacks = self.problem.B @ y_star - (self.problem.A @ x - self.problem.b)
        min_slack = torch.min(slacks).item()
        
        if min_slack > 1e-6:
            # Constraints are inactive - use analytical Jacobian
            if hasattr(self.problem, 'jacobian_dy_dx_if_inactive'):
                J = self.problem.jacobian_dy_dx_if_inactive()
                implicit_component = J.T @ grad_y
            else:
                # Fallback to numerical computation
                implicit_component = self.compute_implicit_gradient_numerical(x, y_star, grad_y)
        else:
            # Constraints are active - use numerical computation
            implicit_component = self.compute_implicit_gradient_numerical(x, y_star, grad_y)
    ...
    def compute_implicit_gradient_numerical(self, x: torch.Tensor, y_star: torch.Tensor, 
        ...
        """
        Numerical implicit gradient computation as fallback
        """
    ...
    def optimize_2000_iterations_corrected_gap(self,
                                             alpha: float,
                                             eta: float,
                                             D: float,
                                             N_g: int,
        ...
        """
        F2CSA.tex theory implementation with corrected gap computation
        """
        ...
        if verbose:
            print(f"Starting F2CSA 2000-iteration optimization (CORRECTED GAP COMPUTATION)")
        ...
        for iteration in range(max_iterations):
            ...
            # Compute gap with corrected computation
            current_gap, direct_norm, implicit_norm = self.compute_gap_corrected(xx)
    ...
    def comprehensive_alpha_optimization_corrected(self, alpha_values: List[float], max_iterations: int = 2000) -> Dict:
        """
        Comprehensive alpha optimization with corrected gap computation
        """
        print("="*80)
        print("COMPREHENSIVE ALPHA OPTIMIZATION WITH CORRECTED GAP COMPUTATION")
        print("="*80)
        ...
        for alpha in alpha_values:
            print(f"\nTesting alpha = {alpha:.3f} for 2000 iterations (CORRECTED GAP)")
            print("-" * 60)
            
            result = self.optimize_2000_iterations_corrected_gap(
                alpha=alpha,
                eta=eta,
                D=D,
                N_g=N_g,
    ...
    def run_corrected_gap_optimization(self):
        """
        Run optimization with corrected gap computation
        """
        print("="*80)
        print("F2CSA CORRECTED GAP COMPUTATION - ACHIEVING SMOOTH AVERAGE GAP < 0.1")
        print("Using analytical Jacobian for inactive constraints")
        print("="*80)
        
        # Comprehensive alpha optimization
        alpha_values = [0.02, 0.05, 0.1, 0.15, 0.2, 0.3, 0.5, 0.7, 1.0]
        alpha_results = self.comprehensive_alpha_optimization_corrected(alpha_values, max_iterations=2000)
        ...
        print(f"  CORRECTED GAP COMPUTATION")
        
        final_result = self.optimize_2000_iterations_corrected_gap(
            alpha=best_alpha,
            eta=1e-3,
            D=0.5,
            N_g=32,
...
def main():
    """
    Main function to run corrected gap computation optimization
    """
    print("Starting F2CSA Corrected Gap Computation Optimization...")
    
    # Create optimization algorithm
    solver = F2CSACorrectedGapComputation(device='cpu', seed=42, verbose=True)
    
    # Run optimization
    results = solver.run_corrected_gap_optimization()
